...
mkdir $output_dir/MAGs/
mv $output_dir/$f.fa $output_dir/MAGs/

###################################################################################################
# Split the wheat from the chaff, archiving the low-quality MAGs (commented out by RJM 01.03.2019)
###################################################################################################
...
###################################################################################################
gtdbtk classify_wf --genome_dir $output_dir/MAGs/ --out_dir $output_dir/gtdbtk_output/ -x .fa --cpus $n_thread
...
for file in $output_dir/MAGs/*.fa;
do
	f=$( basename $file )	
	bin_num=$(cut -d'.' -f2 <<< $f)
